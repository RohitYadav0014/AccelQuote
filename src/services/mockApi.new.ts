...
export const mockExtractPdfData = async (fileId: string): Promise<{ message: ExtractionData }> => {
    ...
    return {
        message: {
            markdown_text: `# RFQ 6000243598\n\nFrom:\n\nTeresa Lobo <teresa.lobo@group-etis.com> Wednesday, March 5, 2025 6:22 AM france, prm [EXTERNAL] RFQ 6000243598...`,
...
export const mockItemsPricing = async (items: ItemInformation[]): Promise<{ message: { csv_data_pricing: string; item_price_details: PriceDetail[] } }> => {
    await new Promise(resolve => setTimeout(resolve, 1000));
    // Return the items_information as JSON in the response for debugging/inspection
    return {
        message: {
            csv_data_pricing: JSON.stringify(items, null, 2),
            item_price_details: mockPriceDetails
        }
    };
};
    ...
    return {
        message: {
            complete_pricing_info: items_information.map((item, index) => {
                ...
                // Use the CNP FACTOR USD value passed in, which is set to the correct currency factor by the UI
                const cnpFactor = cnp_discount_info[index]["CNP FACTOR USD"];
                // Use the Discount Authorization Sales Engineer as a decimal (if > 1, treat as percent)
                let discount = cnp_discount_info[index]["Discount Authorization Sales Engineer"];
                ...
                return {
                    ...
                    "Discount Authorization Sales Engineer": discount,
                    "CNP_Price": cnpPrice,
                    "CNP_with_dicsount": cnpPrice * (1 - discount)
                };
            })
        }
    };
